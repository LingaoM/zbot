...
static int skill_blink_led(const char *arg, char *result, size_t res_len)
{
    ...
    if (arg && arg[0] != '\0') {
        ...
        if (times <= 0 || times > 20) times = 3;
    }

    char tool_result[TOOLS_RESULT_MAX_LEN];

    for (int i = 0; i < times; i++) {
        tools_execute("gpio_write", "{\"pin\":\"led0\",\"value\":1}",
                      tool_result, sizeof(tool_result));
        k_msleep(300);
        tools_execute("gpio_write", "{\"pin\":\"led0\",\"value\":0}",
                      tool_result, sizeof(tool_result));
        k_msleep(300);
    }

    snprintf(result, res_len,
             "Blinked led0 %d times.", times);
    return 0;
}
...
{
    if (!name || !description || !fn) return -EINVAL;
    if (g_skill_count >= SKILL_MAX_COUNT)  return -ENOMEM;
...
{
    if (idx < 0 || idx >= g_skill_count) return NULL;
    return &g_skills[idx];
}

void skills_register_builtins(void)
{
    skill_register("blink_led",
                   "Blink LED0 N times (arg: number of blinks)",
                   skill_blink_led);

    skill_register("sos",
                   "Blink SOS morse code pattern on LED0",
                   skill_sos);

    skill_register("system_status",
                   "Report board uptime, heap usage, and hardware info",
                   skill_system_status);

    skill_register("clear_memory",
                   "Wipe conversation history and NVS summary",
                   skill_clear_memory);
