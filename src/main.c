 ...
 *
 * Boot sequence:
 *   1. Initialise soul (identity, default config — no credentials)
...
int main(void)
{
    ...
    /* 1. Soul — identity and runtime config */
    soul_init();

    /* 2. Memory — NVS mount and load persisted summary */
    rc = memory_init();
    if (rc < 0) {
        LOG_WRN("Memory/NVS init warning: %d (continuing without persistence)",
                rc);
    }

    /* 3. Config — load WiFi credentials + API key from NVS, auto-connect */
    config_init();
    ...
    /* Wire memory compression callback to agent (avoids circular include) */
    memory_set_summary_callback(agent_request_summary);

    /* 6. Register WiFi event callback */
    ...
    while (1) {
        ...
        if (g_wifi_connected) {
            LOG_DBG("Heartbeat — WiFi up, agent %s",
                    agent_is_busy() ? "busy" : "idle");
        }
    }
