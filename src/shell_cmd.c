 ...
 *   claw status                  -- Show current soul/config status
...
static int cmd_key(const struct shell *sh, size_t argc, char **argv)
{
    ...
    int rc = soul_set_api_key(argv[1]);
    if (rc == 0) {
        shell_print(sh, "API key set (%zu chars). RAM only — use 'claw key-save' to persist.",
                    strlen(argv[1]));
    } else {
...
static int cmd_key_save(const struct shell *sh, size_t argc, char **argv)
{
    ...
    int rc = config_save_api_key(soul_get()->api_key);
    if (rc == 0) {
...
static int cmd_host(const struct shell *sh, size_t argc, char **argv)
{
    ...
    int rc = soul_set_endpoint_host(argv[1]);
    if (rc == 0) {
...
static int cmd_path(const struct shell *sh, size_t argc, char **argv)
{
    ...
    int rc = soul_set_endpoint_path(argv[1]);
    if (rc == 0) {
...
static int cmd_provider(const struct shell *sh, size_t argc, char **argv)
{
    ...
    int rc = soul_set_provider_id(argv[1]);
    if (rc == 0) {
...
static int cmd_tls(const struct shell *sh, size_t argc, char **argv)
{
    ...
    bool use_tls = (strcmp(argv[1], "on") == 0 ||
                    strcmp(argv[1], "1")  == 0  ||
                    strcmp(argv[1], "yes") == 0);
...
static int cmd_status(const struct shell *sh, size_t argc, char **argv)
{
    ...
    ARG_UNUSED(argv);
    soul_print_status();
    shell_print(sh, "Memory: %d turns in RAM",
                memory_get_turn_count());
    shell_print(sh, "Agent busy: %s",
                agent_is_busy() ? "yes" : "no");
    return 0;
}
...
{
    struct chat_work_item *item =
        CONTAINER_OF(work, struct chat_work_item, work);
    item->rc = agent_run_sync(item->input, item->response,
...
static int cmd_summary(const struct shell *sh, size_t argc, char **argv)
{
    ARG_UNUSED(argc);
    ARG_UNUSED(argv);
    const char *s = memory_get_summary();
    shell_print(sh, "NVS Summary: %s", s[0] ? s : "(none)");
    return 0;
}
...
static int cmd_skill_run(const struct shell *sh, size_t argc, char **argv)
{
    ...
    int rc = skill_run(argv[1], arg, result, sizeof(result));
...
static int cmd_tools(const struct shell *sh, size_t argc, char **argv)
{
    ...
    int count;
    const struct tool_descriptor *tools = tools_get_all(&count);

    shell_print(sh, "=== ZephyrClaw Tools (%d) ===", count);
    for (int i = 0; i < count; i++) {
        shell_print(sh, "  %-20s %s",
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_claw,
    SHELL_CMD_ARG(key,        NULL, "Set API key (RAM only): key <key>",
                  cmd_key, 2, 0),
    SHELL_CMD(key_save,       NULL, "Save API key to flash",
              cmd_key_save),
    SHELL_CMD(key_delete,     NULL, "Delete API key from flash",
              cmd_key_delete),
    SHELL_CMD_ARG(host,    NULL, "Set LLM host: host <hostname>",
                  cmd_host, 2, 0),
    SHELL_CMD_ARG(path,    NULL, "Set API path: path <path>",
                  cmd_path, 2, 0),
    SHELL_CMD_ARG(model,    NULL, "Set model: model <name>",
                  cmd_model, 2, 0),
    SHELL_CMD_ARG(provider, NULL, "Set X-Model-Provider-Id: provider <id>",
                  cmd_provider, 2, 0),
    SHELL_CMD_ARG(tls,      NULL, "Configure TLS: tls <on|off> [port]",
                  cmd_tls, 2, 1),
    SHELL_CMD(status,      NULL, "Show current configuration",
              cmd_status),
    SHELL_CMD_ARG(chat,    NULL, "Chat with the agent: chat <message>",
                  cmd_chat, 2, 32),
    SHELL_CMD(history,     NULL, "Show conversation history",
              cmd_history),
    SHELL_CMD(summary,     NULL, "Show NVS persisted summary",
              cmd_summary),
    SHELL_CMD(clear,       NULL, "Clear in-RAM conversation history",
              cmd_clear),
    SHELL_CMD(wipe,        NULL, "Wipe all history and NVS summary",
              cmd_wipe),
    SHELL_CMD(skill,       &sub_skill, "Skill commands", NULL),
    SHELL_CMD(tools,       NULL, "List available tools",
              cmd_tools),
    SHELL_SUBCMD_SET_END
);
